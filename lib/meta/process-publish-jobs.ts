...
type ClaimedPublishJob = {
  ...
  duplicatedFromAdId: string | null;
...
async function claimNextPublishJob(companyId?: string): Promise<ClaimedPublishJob | null> {
...
async function claimPublishJobBatch(
  count: number,
  companyId?: string,
): Promise<ClaimedPublishJob[]> {
  const claimed = await Promise.all(
    Array.from({ length: count }, () => claimNextPublishJob(companyId)),
  );
  return claimed.filter((job): job is ClaimedPublishJob => job !== null);
}

async function processOneClaimedPublishJob(
  job: ClaimedPublishJob,
): Promise<ProcessedPublishJob> {
  try {
...
export async function processPublishJobs(input: {
  ...
  const concurrency = Math.min(5, Math.max(1, input.concurrency ?? 1));
  const processed: ProcessedPublishJob[] = [];

  let remaining = limit;
  while (remaining > 0) {
    const batchSize = Math.min(remaining, concurrency);
    const jobs = await claimPublishJobBatch(batchSize, input.companyId);
    if (jobs.length === 0) break;

    const batchResults = await Promise.allSettled(
      jobs.map((job) => processOneClaimedPublishJob(job)),
    );
    ...
    for (const result of batchResults) {
      if (result.status === 'fulfilled') {
        ...
        console.error('[process-publish-jobs] unexpected batch failure', result.reason);
      }
    }
...
export async function runPublishWorkerForCompany(
  companyId: string,
  limit = 10,
  options?: { concurrency?: number },
) {
  return processPublishJobs({ limit, companyId, concurrency: options?.concurrency });
}
