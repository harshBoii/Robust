...
- **geo.publish_content** — Publish draft(s) live. Args: bountyId, platform and/or approveAll, optional contentId, redditSubreddit/redditFlairId for REDDIT if not already in session, optional \`blogDestination\` (\`shopify\` | \`wordpress\` | \`nextjs\`) for WEBSITE_BLOG. **No manual user approval required** — publish when you are confident it matches user intent.
  - If a WEBSITE_BLOG publish fails because more than one blog destination (Shopify, WordPress, Next.js site) is connected, call **geo.get_publish_targets**, then ask the user which one to use and retry with \`blogDestination\`.
