...
import { syncGoogleCampaigns, createAndStoreGoogleCampaignFromPreset } from '@/lib/google-ads/sync';
...
export async function GET() {
  ...
  const campaigns = await syncGoogleCampaigns(integration.id);
...
export async function POST(req: NextRequest) {
  ...
  const preset = await prisma.googleCampaignPreset.findFirst({
    where: { id: presetId },
    select: { id: true },
  });
  ...
  try {
    const campaign = await createAndStoreGoogleCampaignFromPreset({
