...
import { enableTwoFactor, getCompanyTwoFactorSecret } from '@/lib/auth/two-factor';
...
export async function POST(request: Request) {
  ...
  const secret = await getCompanyTwoFactorSecret(session!.companyId);
  if (!secret) {
    return NextResponse.json({ error: 'Run 2FA setup first' }, { status: 400 });
  }

  const ok = await enableTwoFactor(session!.companyId, secret, code);
  if (!ok) {
